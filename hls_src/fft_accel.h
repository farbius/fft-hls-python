 ...
 */
template <int U>
ap_uint<U> revBits(ap_uint<U> Addr)
{
	ap_uint<U> revAddr = 0;
	for(uint8_t idx = 0; idx < U; idx ++)
	{
		revAddr <<= 1;
		revAddr |= Addr & 1;
		Addr >>= 1;
...
{
#pragma HLS INLINE
	L1:for(uint16_t idx = 0; idx <  NPTS; idx ++)
...
void n_stage(T x[NPOINTS], T y[NPOINTS], T w[NPOINTS], uint8_t casc)
{
	T x0 = 0, y0 = 0;
	T x1 = 0, y1 = 0;
	L1:for(uint16_t idx = 0; idx < NPOINTS / 2; idx ++)
	{

		uint16_t d = ((idx % POW2(casc)) == 0)  ? 2*idx : d + 1;

		uint16_t _idx1   = revBits<FFTRADIX>((ap_uint<FFTRADIX>)(d + 0));
		uint16_t _idx2   = revBits<FFTRADIX>((ap_uint<FFTRADIX>)(d + POW2(casc)));
		uint16_t _widx   = W_IDX(idx, casc);

		x0 = x[_idx1];
		y0 = x[_idx2];

		butter_dit<T, U, V, 15>(x0, y0, w[_widx], &x1, &y1);
...
void wrapped_fft_hw (stream<stream_1ch> &in_stream, stream<stream_1ch> &out_stream)
{
	...
	pop_input  <T, TU, TI, TD, NPOINTS>( in_stream, x0[0]);

	for(uint8_t casc = 0; casc < FFTRADIX; casc ++)
		n_stage    <T,U,V>(  x0[casc], x0[casc + 1], w0, casc);

	push_output<T, TU, TI, TD, NPOINTS>(out_stream, x0[FFTRADIX]);
